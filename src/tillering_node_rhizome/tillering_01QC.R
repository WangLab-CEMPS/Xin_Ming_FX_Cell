...
# 创建Seurat对象
sample <- c(
  tillering = "./rawdata/tillering_node"
)
...
# 添加细胞器基因比例
Mt <- read.table("~/ref/Oryza_sativa/annotation/Mt_gene.txt")[[1]]
Pt <- read.table("~/ref/Oryza_sativa/annotation/Pt_gene.txt")[[1]]
...
(p <- qc_plot(obj, max_nFeature = 6000, min_nFeature = 500, max_nCount = 25000))
...
table(
  obj$nFeature_RNA > 500 &
    obj$nFeature_RNA < 6000 &
    obj$nCount_RNA < 25000 &
    obj$percent.mito < 5 & obj$percent.chlo < 2
)
...
# 保存
saveRDS(obj, paste0("./dataLib/tillering_node_rhizome/tillering.rds"))
