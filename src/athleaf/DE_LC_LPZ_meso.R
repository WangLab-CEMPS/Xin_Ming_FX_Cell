...
deg %>%
  ...
  write.csv("./dataLib/athLeaf/LC_LPZ_Mesophyll_de.csv", row.names = FALSE) %>%
  subset(p_val < 0.05 & p_val_adj < 0.05 & abs(avg_log2FC) > 2) -> dd

dd %>%
  mutate(sig = ifelse(avg_log2FC > 0, "Up", "Down")) %>%
  write.csv("./dataLib/athLeaf/LC_LPZ_Mesophyll_de_sig.csv", row.names = FALSE)
...
write.csv(
  as.data.frame(go_up),
  "./dataLib/athLeaf/LC_LPZ_Mesophyll_go_up.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  "./dataLib/athLeaf/LC_LPZ_Mesophyll_go_down.csv",
  row.names = FALSE
)
...
p1 <- dotplot(go_up, showCategory = 7, label_format = 100) + ggtitle("Up")
p2 <- dotplot(go_down, showCategory = 15, label_format = 100) + ggtitle("Down")

p <- p1 / p2 + plot_layout(height = c(1, 1.5))

pdf("./figures/leaf/LC_LPZ_Mesophyll_de_go.pdf", width = 8, height = 8)
p
dev.off()
