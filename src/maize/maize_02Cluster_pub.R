...
ggsave(filename = "./Plots/maize/maize_pub_umap.pdf", p_umap, width = 7, height = 7.5)
...
## Epidermis      Zm00001eb255380 # Zm00001eb137050
# QC              Zm00001eb200450
...
obj$celltype[obj$celltype %in% c("17", "13", "10")] <- "Endodermis"
obj$celltype[obj$celltype %in% c("9", "6", "11")] <- "Phloem & Sieve"
obj$celltype[obj$celltype %in% c("18")] <- "Root Cap"
obj$celltype[obj$celltype %in% c("14")] <- "Xylem"
obj$celltype[obj$celltype %in% c("12")] <- "Cortex Initials"
obj$celltype[obj$celltype %in% c("5")] <- "CortexII(m)"
obj$celltype[obj$celltype %in% c("0", "3", "2")] <- "Cortex"
obj$celltype[obj$celltype %in% c("7")] <- "Epidermis"
obj$celltype[obj$celltype %in% c("1")] <- "QC"
obj$celltype[obj$celltype %in% c("15", "16")] <- "Initials CC"
obj$celltype[obj$celltype %in% c("19", "8", "4")] <- "Pericycle"
